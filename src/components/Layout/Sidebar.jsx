...
const Sidebar = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className="fixed top-8 left-8 bottom-8 w-24 md:w-80 bento-glass rounded-[2.5rem] z-50 hidden md:flex flex-col p-6 border-white/10 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.6)]">
      ...
      <div className="fixed bottom-6 left-6 right-6 h-20 bento-glass rounded-[2rem] z-50 flex md:hidden items-center justify-around px-4 border-white/10 shadow-2xl">
        {menuItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          ...
          return (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className="relative flex flex-col items-center justify-center w-14 h-14 rounded-2xl transition-all duration-300 outline-none"
            >
              <div className={`transition-all duration-300 ${isActive ? 'text-primary scale-110' : 'text-slate-500'}`}>
                <Icon size={24} strokeWidth={isActive ? 2.5 : 2} />
              </div>
              {isActive && (
                <motion.div
